...
function renderAll() {
  ...
  const critical = checks.filter((c) =>
    ["video", "algo", "abstraction", "list", "selection", "iteration"].includes(
      c.id,
    ),
  );
  document.getElementById("overviewChecks").innerHTML = critical
    .map(reqCard)
    .join("");
  document.getElementById("allRequirements").innerHTML = checks
    .map(reqCard)
    .join("");

  renderRubric(checks);

  // populate Program Code tab listing (visible on screen + printable)
  const listing = document.getElementById("print-code-listing");
  if (listing) {
    const files = Object.entries(codeContent);
    if (files.length) {
      const CHUNK = 48; // lines per unbreakable chunk
      listing.innerHTML = files
        .map(([name, src]) => {
          ...
          const chunkedBody = chunks
            .map((c) => `<div class="pcl-chunk">${c}</div>`)
            .join("");
      ...
      listing.innerHTML =
        '<div class="empty"><div class="empty-icon">⊘</div><p>No code files found.</p></div>';
    }
  }
}
  ...
  const rubricRows = [
    {
      row: "Row 1",
      pts: 1,
      skill: "Program Purpose & Function",
    ...
    },
    {
      row: "Row 5",
      pts: 1,
      skill: "Algorithm Implementation",
...
function updateDashRow(i, result, url) {
  ...
  const color = statusColor(pct >= 80 ? "pass" : pct >= 50 ? "warn" : "fail");
  const pillCls =
    pct >= 80 ? "pill-pass" : pct >= 50 ? "pill-warn" : "pill-fail";
  const dot = (id) => {
...
function updateClassStats() {
  ...
  const avg = Math.round(
    batchResults.reduce(
      (s, r) =>
        s +
        (r.checks.filter((c) => c.status === "pass").length / r.checks.length) *
          100,
      0,
    ) / n,
  );
