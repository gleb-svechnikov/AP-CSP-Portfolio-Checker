...
function toggleBatch() {
  const a = document.getElementById('batchArea');
...
function updateDashRow(i, result, url) {
  ...
  row.innerHTML = `
    ...
    <td><span class="pill ${pillCls}">${passed}/${checks.length}</span><div class="mini-bar"><div class="mini-fill" style="width:${pct}%;background:${color}"></div></div></td>
...
function updateClassStats() {
  ...
  if (!n) return;
  const avg = Math.round(batchResults.reduce((s, r) => s + (r.checks.filter(c => c.status === 'pass').length / r.checks.length * 100), 0) / n);
...
function checkRequirements(data, code) {
  ...
  res.push({ id:'readme', title:'README / written project description', category:'Artifacts',
    ...
    detail: hasReadme ? (readme.length > 100 ? 'README found with substantial content.' : 'README found but very short — expand to describe purpose, inputs, outputs, and features shown in the video.') : 'No README found.' });
...
function renderAll() {
  ...
  const updated = info.updated_at ? new Date(info.updated_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : '—';
...
function renderCode() {
  ...
  document.getElementById('codePanel').innerHTML = codeFiles.map(f => {
    ...
    const preview = txt ? escHtml(txt.slice(0, 2500)) + (txt.length > 2500 ? '\n…' : '') : '<em style="color:var(--muted)">Could not load file content.</em>';
    return `
      <div class="code-file">
